...
const StepsWrap = styled.div`
  ...
  @media (max-width: ${theme.breakpoints.md}) { flex-direction: column; gap: 40px; }
`
...
const processSteps = [
  ...
  { num: '02', name: 'Research & Strategy', desc: '사용자 인터뷰, 경쟁사 분석, 데이터 분석을 통해 전략 방향을 수립합니다.' },
    ...
    <>
      <PageHead>
        <Container>
          ...
          <PageSub>
            매 프로젝트에서 동일한 기준으로 일합니다.<br />
            과정이 결과의 품질을 만듭니다.
          </PageSub>
          <ChipRow>
            {['KPI-First Design', 'Design Rationale', 'System Builder', 'Dev-Aware', 'B2B·SaaS·Landing·Brand'].map((c) => (
              <Chip key={c}>{c}</Chip>
            ))}
          </ChipRow>
        </Container>
          ...
          <DiffGrid>
            {diffCards.map((c, i) => (
              <DiffCard key={i}>
                ...
                <DiffChipsRow>
                  {c.chips.map((chip) => <DiffChip key={chip}>{chip}</DiffChip>)}
                </DiffChipsRow>
              </DiffCard>
            ))}
          </DiffGrid>
      ...
      <section style={{ padding: '80px 0' }}>
        <Container>
          <CtaBlock>
            ...
            <CtaDesc>
              FCTS의 디자인은 단순한 시각적 결과물이 아닙니다. 설계 이유가 있고, 근거가 있으며,
              팀이 이어받아 운영할 수 있는 시스템입니다.
            </CtaDesc>
          </CtaBlock>
        </Container>
      </section>
