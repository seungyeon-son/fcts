...
const ValueCards = styled.div`
  ...
  gap: 16px;
  margin-top: 48px;
  @media (max-width: ${theme.breakpoints.md}) { flex-wrap: wrap; }
`
...
const ProjectRow = styled.div`
  ...
  &:last-child { border-bottom: 1px solid #e8e6e0; }
...
const skills = [
  'Figma', 'Adobe Illustrator', 'Framer', 'HTML/CSS', 'Git',
  'UX Research', 'Design System', 'Prototyping', 'AutoCAD',
]
    ...
    <>
      ...
      <section style={{ paddingBottom: '80px' }}>
        <Container>
          ...
          <ProfileDesc>
            복잡한 비즈니스 문제를 데이터와 시스템 사고로 해결하는 UX 디자이너입니다.
            B2B SaaS, 이커머스, 교육 플랫폼 등 다양한 도메인에서 사용자 경험을 설계해왔으며,
            디자인 시스템 구축과 운영 원칙 정립을 통해 팀 전체의 설계 효율을 높이는 것에
            관심이 많습니다. 완성된 화면이 아닌 작동하는 시스템을 납품하는 것을 목표로 합니다.
          </ProfileDesc>
          <SkillsRow>
            {skills.map((s) => <Skill key={s}>{s}</Skill>)}
          </SkillsRow>
        </Container>
      </section>
      ...
      <section style={{ padding: '80px 0' }}>
        <Container>
          ...
          {mainProjects.map((p, i) => (
            <ProjectRow key={i}>
              <div>
                <Link href={p.href}><ProjName>{p.name}</ProjName></Link>
                <ProjCat>{p.category}</ProjCat>
                <ProjChipsRow>
                  {p.tags.map((t) => <ProjChip key={t}>{t}</ProjChip>)}
                </ProjChipsRow>
              </div>
